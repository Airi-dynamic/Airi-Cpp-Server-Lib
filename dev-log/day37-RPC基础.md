# Day 37 — RPC 基础层：二进制帧协议 / RpcServer / RpcClient

> **今日目标**：为后续 Raft 选举实现可复用的 RPC 通信层。  
> **背景**：Day 30 结束后项目转向分布式存储方向（见 `AGENT_MEMORY/PLAN.md`）。Raft 节点间需要互发 RequestVote / AppendEntries 两种 RPC；这些调用频率低（< 100/s）、要求低延迟（< election timeout），最适合用**短连接同步阻塞**模型。  
> **基于**：Day 30（全特性 HTTP 服务器）。**结束**：分布式存储 Phase 1 第一步。下一步（Day 38）是 Raft 选主。
...
接下来 Day 38 会在今天的 RPC 层之上实现 **Raft 选主**：
